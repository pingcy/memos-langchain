"""
LangChain 1.0 智能体示例 V2 - 带 MemOS 长期记忆
==============================================

功能：
1. 使用 create_agent + Tavily 搜索工具进行网络搜索
2. 使用 MemOS V2 中间件实现长期记忆能力（基于 SingleCubeView API）
3. 智能体可以记住用户的偏好、历史对话等信息

V2 变更说明:
- 使用 memos_middleware_v2（基于 init_server + SingleCubeView）
...
# 导入 MemOS V2 中间件
from memos_middleware_v2 import create_memos_middleware_v2
...
# 创建 MemOS V2 记忆中间件
# 这将使智能体具备长期记忆能力
memos_middleware = create_memos_middleware_v2(
    user_id="langchain_agent_user_v2",  # 用户ID，用于隔离不同用户的记忆
    cube_id="langchain_agent_cube_v2",   # Cube ID，用于隔离不同的记忆空间
    top_k=5,                              # 每次检索返回的记忆数量
...
- 请用中文回答问题""",
    middleware=[memos_middleware],  # 注入 MemOS V2 中间件
)

# 运行智能体
if __name__ == "__main__":
    print("=" * 60)
    print("LangChain 1.0 智能体 V2 - Tavily 搜索 + MemOS 长期记忆")
