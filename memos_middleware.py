...
class MemosMiddleware(AgentMiddleware):
    ...
    def __init__(
        ...
        # 初始化 MemOS 记忆助手
        self.memory_helper = MemosMemoryHelper(
            user_id=user_id,
            top_k=top_k
        )
    ...
    def _log(self, message: str):
        """打印日志"""
        if self.verbose:
            print(f"🧠 [MemosMiddleware] {message}")
...
if __name__ == "__main__":
    # 测试中间件创建
    middleware = create_memos_middleware(user_id="test_middleware_user")
    print(f"中间件创建成功，当前记忆数量: {middleware.get_memory_count()}")
